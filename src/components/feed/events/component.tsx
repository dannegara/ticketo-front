import React from 'react';
import EventPreview from '../../UI/event-preview';

const dummyLogo: string = "data:image/png;base64,";


interface IEventProps{
    events: object[]
}

const Events: React.SFC<IEventProps> = (props) => {
    return(
        <div>
            {props.events.map((event: any) => (
                <EventPreview 
                    key={event.eventId}
                    logo={dummyLogo}
                    organizerName={`${event.organizer_firstname} ${event.organizer_lastname}`}
                    organizerLink={`${event.organizer_firstname}_${event.organizer_lastname}`}
                    eventName={event.title}
                    eventDate={event.event_date}
                    description={event.description}
                    price={event.price}
                    eventId={event.eventId}
                    posterUrl={event.poster}
                />
            ))}
        </div>
    )
}

export default Events;